...
c------------------------------------------------------------------
	...
	real*4 radprb,rad,temp,bran,rmidg,gran,rscale
	real*4 clevel
c------------------------------------------------------------------
	integer iargc,ip,nrec
	integer midg,nsurf,ineigh
	integer i,j,k
	real*4 expand  ! globally increase radius 23 jan 07, undone nov 2007 rgc
	data expand / 0.2 /
c------------------------------------------------------------------
c
...
c
	print *,'meshsrfA aug 2007 grid size: ',ngrid
	print *,'changed epsmp to real'
	print *,'use contour level, set to 0.5'
	print *,'interpolating cutpoints'
	radprb = 1.4
	clevel = 0.6
	if(iargc().lt.1)then
	  print * ,'Usage: meshsrf pdb_file [probe_radius] [scale_factor]'
	else if(iargc().ge.2)then
	  call getarg(2,probe)
	  read(probe,'(f5.2)',err=904)radprb
	  if(iargc().gt.2)then
	    call getarg(2,probe)
	    read(probe,'(f5.2)',err=904)rscale
	  endif
	endif
...
c
c write phimap of in/out map
c
      open(21,file='meshsrfA.phi',form='unformatted')
      phead = 'meshsrfA  '
      ntitle = 'from meshsrf'
...
c	    print *,iz1
		...
		do ix1 = 2,ngrid-1
c		if(teps(ix1,iy1,iz1).eq.1)then
	          ...
	          ixmax = amin1(coord,rgrid)
                do iz =  izmin,izmax
                  do iy =  iymin,iymax
                    do ix =  ixmin,ixmax
              d2 = (ix-xn(1))**2 + (iy-xn(2))**2 + (iz-xn(3))**2
c                      if(d2.le.radp2) then
c                        epsmp(ix,iy,iz) = 0.
c                      end if                  
c interpolate aug 07 kas
                      if(d2.le.radp2) then
			      dist = sqrt(d2)
				val = dist/(2.*radp)
				epsmp(ix,iy,iz) = min(val,epsmp(ix,iy,iz))
                      end if                  
c interpolate aug 07 kas
			  end do
			end do
...
c sqrt 3. since mean orientation of surface elements of sphere 
c is at angle to grid
	print *,'probe place points done: ',ndo
	surf = nsurf/(sqrt(3.)*scale**2)  
...
c	  if(l.eq.1)then
c	    xn(1,it) = xn(1,it) + 0.5/scale
c	  else if(l.eq.2)then
c	    xn(2,it) = xn(2,it) + 0.5/scale
c	  else if(l.eq.3)then
c	    xn(3,it) = xn(3,it) + 0.5/scale
c	  end if
        eps1 = epsmp(i,j,k)
        if(l.eq.1)then
          eps2 = epsmp(i+1,j,k)
