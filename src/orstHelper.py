...
from rangesearch import orthoRangeSearchTree #my rangesearch
import geometry
import operator

#def comp10(itemA, itemB):
#  return cmp(itemA[10], itemB[10])          #sort so all 3 together

def getIntersectingPts(startPt, endPt, longEdge, shortAxis1,  \
                       shortAxis2, orst, maxIntersect):
  ...
  try: #to catch divide by zero issues
    ...
    for point in checkResults:
      if False == maxIntersect or len(intersectingPts) < maxIntersect:
        ...
        if ptCount == 3:
          ...
          short1BelowCt, short1AboveCt, short2BelowCt, short2AboveCt = 0,0,0,0
          ...
          if short1BelowCt > 0 and short1AboveCt > 0 and \
            ...
            #print triPts0,triPts1,triPts2,startPt,endPt
            triPts0 = triTuple[0]
            triPts1 = triTuple[1]
            triPts2 = triTuple[2]
    ...
    #tstdebug.debugTris([startPt,endPt],intersectingTris,intersectingPts,"debug."+str(x)+"."+str(y)+"."+str(z)+".line.py")
...
def buildOrst(triTuples, shortAxis1, shortAxis2, limitPoints, \
              pointTriList, pointList):
  ...
  for ptIndex, pointTri in enumerate(pointTriList):
    if pointTri[0] in limitPoints:
      ...
      inputData[ptIndex] = [pointXYZ[1],pointXYZ[2],pointXYZ[3],modifiedTriList]
  newInputData = []
  for inputRow in inputData: 
    if inputRow != False:
      newInputData.append(inputRow)
  if len(newInputData) > 0:
    orst = orthoRangeSearchTree(newInputData, shortAxis1, shortAxis2)
    return orst
  else:
    return False
    
def decideInsideLong(emptyGrid, triTuples, longAxis, allPoints, \
             pointTriList, pointList, triList, valueToSet, \
             valueFromSet=False, maxIntersect=False):
  ...
  orst = buildOrst(triTuples, shortAxis1, shortAxis2, allPoints, \
                   pointTriList, pointList)
  if False != orst:
    for short1 in range(0, lens[shortAxis1]):
      ...
      for short2 in range(0, lens[shortAxis2]):
        ...
        while False == intersectingPts:
          intersectingPts = getIntersectingPts(usedStart, usedEnd, longEdge, \
                  shortAxis1, shortAxis2, orst,maxIntersect)
          usedStart, usedEnd = geometry.perturbLine(longAxis, shortAxis1, shortAxis2, \
                                           usedStart, usedEnd, maxIt)
          maxIt -= 1
          if maxIt < 0:
        ...
        if len(intersectingPts) > 0:
          ...
          longInts = [xb[longAxis] for xb in intersectingPts] #make quick array
          longInts.sort()
          ...
          for longCount in range(0, lens[longAxis]):
            ...
            if lInside: #replace the encoding int
              if False == valueFromSet or valueFromSet == gridPiece[0]:
                newGridPiece = valueToSet,gridPiece[1],gridPiece[2],gridPiece[3]
                emptyGrid[x][y][z] = newGridPiece
  #emptyGrid has been modified, nothing returned...
        
def decideInside(emptyGrid, triTuples, allPoints, \
            pointTriList, pointList, triList, valueToSet, \
            valueFromSet=False, maxIntersect=False):
  ...
  if lenZ >= lenX and lenZ >= lenY:
    decideInsideLong(emptyGrid,triTuples,2,allPoints,pointTriList,pointList, \
                     triList, valueToSet, valueFromSet,maxIntersect)
  elif lenY >= lenX and lenY >= lenZ:
    decideInsideLong(emptyGrid,triTuples,1,allPoints,pointTriList,pointList, \
                     triList, valueToSet, valueFromSet, maxIntersect)
  elif lenX >= lenZ and lenX >= lenY:
    decideInsideLong(emptyGrid,triTuples,0,allPoints,pointTriList,pointList, \
                     triList, valueToSet, valueFromSet, maxIntersect)
