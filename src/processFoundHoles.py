...
import sys, string
import statistics 
import comparePaths, paths #get path stats methods
...
colNamesBonus = ["stepLength", "pathLength", "pathMinRadius", "pathMaxInsideRadius", "pRMSD", "coverage", "span", "wRMSD", "less1", "lessRad", "radiicomp"]
...
def processData(dataList, nameList, listPaths, \
  ...
  for colIdx, sortColNumber in enumerate(compCols):
    ...
    for index,colName in enumerate(colNames):
      fileOut = open(outputFileName + "rankings." + colName + "." + colNamesBonus[sortColNumber-4] + ".log", 'w')
      for line in sortedEvals:
        fileOut.write(line[index] + "\n")
      fileOut.close()    
      fileOut = open(outputFileName + "rankings.reverse." + colName + "." + colNamesBonus[sortColNumber-4] + ".log", 'w')
  ...
  for column in range(4,14):
    ...
    for colIdx, sortColNumber in enumerate(compCols):
      ...
      for data in dataList:
        if comps[colIdx] == 'min':
          bestVal,bestIndex = getMinColumn(data, sortColNumber)  #index 8 is the prmsd
          ...
          bestVal,bestIndex = getMaxColumn(data, sortColNumber)  #index 8 is the prmsd
