...
class tstData(object):
  ...
  necessaryKeysForMesh = ['CONVEX_HULL_TRI_POINT_LIST', \
                           'CONVEX_HULL_POINT_TRI_LIST', \
                           'POINT_XYZ', \
                           'TRIANGLE_POINT', \
                           'POINT_NEIGHBOR']
  necessaryKeysForPocket = ['CONVEX_HULL_TRI_POINT_LIST', \
                           'CONVEX_HULL_POINT_TRI_LIST', \
                           'POINT_XYZ', \
                           'TRIANGLE_POINT', \
                           'POINT_NEIGHBOR', \
                           ...
                           'PDB_RECORD', \
                           'POINT_PDB_RECORD']
  necessaryKeysForSV = ['POINT_XYZ', \
                        'TRIANGLE_POINT', \
                        'NORM_XYZ', \
                        'POINT_NEIGHBOR', \
                        'CONVEX_HULL_TRI_POINT_LIST' ]
  necessaryKeysForHoles = ['CONVEX_HULL_TRI_POINT_LIST', \
                           'CONVEX_HULL_POINT_TRI_LIST', \
                           'POINT_XYZ', \
                           'NORM_XYZ', \
                           'TRIANGLE_POINT', \
  ...
  necessaryKeysForDisp = ['TRIANGLE_POINT','POINT_XYZ','NORM_XYZ', \
                          ...
                          'HYDROPHOBIC_XYZ', \
                          'POINT_PDB_RECORD']
  necessaryKeysForCurve = ['POINT_XYZ', \
                           'TRIANGLE_POINT', \
                           'POINT_TRIANGLE', \
                           'POINT_NEIGHBOR']
  ...
  def buildNeighbors(self, allPoints):
    ...
    if self.__neighbors == False or 0 == len(self.__neighbors): #remake
      self.__neighbors = {}
...
def writeEntryIntegers(data,title,endtitle,file):
  #file should be open already, data is the data, title and endtitle are the headers
  file.write(title + "\n")
  for line in data:
    lineOut = ""
    for entry in line:
...
class tstDataWritable(tstData):
  def __init__(self,filename,necessaryKeys=False,drawCountTmp=0):
    ...
    if necessaryKeys: #need to add ones we need to write the data back out
      necessaryKeys.extend(['TRIANGLE_NEIGHBOR', 'POINT_XYZ', 'TRIANGLE_POINT', \
           'POINT_TRIANGLE', 'POINT_NEIGHBOR', 'NORM_XYZ', 'POINT_PDB_RECORD', \
           'TRIANGLE_PDB_RECORD', 'PDB_RECORD', 'CURVATURE_XYZ', 'PROPERTY_XYZ'])
    if filename:
      if filename.endswith("bz2"):
        bzTstFile = bz2.BZ2File(filename, 'r')
        records, recordNames = self.readFile(bzTstFile, necessaryKeys)
  ...
  def write(self, filename):
    ...
    writeEntryIntegers(self.dict['TRIANGLE_NEIGHBOR'], \
        "TRIANGLE_NEIGHBOR LIST (CLOCKWISE)", \
        "END TRIANGLE_NEIGHBOR LIST",tstFile)
    writeEntryFloats(self.dict['POINT_XYZ'], \
        "POINT_XYZ LIST", \
        "END POINT_XYZ LIST",tstFile)
    writeEntryIntegers(self.dict['TRIANGLE_POINT'], \
        "TRIANGLE_POINT LIST (CLOCKWISE really, this was checked)", \
        "END TRIANGLE_POINT LIST",tstFile)
    writeEntryIntegers(self.dict['POINT_TRIANGLE'], \
        "POINT_TRIANGLE LIST (CLOCKWISE)", \
        "END POINT_TRIANGLE LIST",tstFile)
    writeEntryIntegers(self.dict['POINT_NEIGHBOR'], \
        "POINT_NEIGHBOR LIST (CLOCKWISE)", \
        "END POINT_NEIGHBOR LIST (CLOCKWISE)",tstFile)
    writeEntryFloats(self.dict['NORM_XYZ'], \
        "NORM_XYZ LIST", \
        "END NORM_XYZ LIST",tstFile)
    writeEntryIntegers(self.dict['POINT_PDB_RECORD'], \
        "POINT_PDB_RECORD", \
        "END POINT_PDB_RECORD",tstFile)
    writeEntryIntegers(self.dict['TRIANGLE_PDB_RECORD'], \
        "TRIANGLE_PDB_RECORD", \
        "END TRIANGLE_PDB_RECORD",tstFile)
    writeEntryString(self.dict['PDB_RECORD'], \
        "PDB_RECORD", "END PDB_RECORD", tstFile)
    writeEntrySingleFloat(self.dict['CURVATURE_XYZ'], \
        "CURVATURE_XYZ", \
        "END CURVATURE_XYZ",tstFile)
    writeEntrySingleFloat(self.dict['PROPERTY_XYZ'], \
        "PROPERTY_XYZ", \
