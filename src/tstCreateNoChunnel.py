...
if -1 != string.find(sys.argv[0], "tstCreateNoChunnel.py"):
  if 6 < len(sys.argv):
    ...
    probe = float(sys.argv[4])
    runTravelDepthCompletely(
        pdbFileName, gridSize, whichSurface, probe, check2trisPerEdge=False,
        doTravelDepth=False)
  elif 3 < len(sys.argv):
    pdbFileName = sys.argv[1]
    gridSize = sys.argv[2]
    whichSurface = sys.argv[3]
    runTravelDepthCompletely(
        pdbFileName, gridSize, whichSurface,
        check2trisPerEdge=False, doTravelDepth=False)
  elif 2 < len(sys.argv):
    pdbFileName = sys.argv[1]
    gridSize = sys.argv[2]
    runTravelDepthCompletely(
        pdbFileName, gridSize, check2trisPerEdge=False, doTravelDepth=False)
  elif 1 < len(sys.argv):
    pdbFileName = sys.argv[1]
    runTravelDepthCompletely(
        pdbFileName, check2trisPerEdge=False, doTravelDepth=False)
  else:
    print "Usage: tstCreateNoChunnel.py file.pdb " + \
