...
def figureOutEdge(curBox, curDist, oldEncode, currentBoxes, \
                  history, adjBox, gapDist, surfEdge=False):
...
def calcTravelDist(gridD, pointList, gridSize, mins, maxs, \
                   allPoints,  extraEdges, surfaceEdgeBoxes, \
                   filename="default.tst", maxDistanceCutoff=False, \
                   volumePoints=False):
  ...
  while (not maxDistanceCutoff or currentTravelDist <= maxDistanceCutoff) \
    ...
    if (curBox[1] == 0 or (curBox[1] > 0 and oldCurEncode > curBox[1]) \
       ...
       and (oldCurEncode > -2 or surfaceEdgeBoxes.has_key(curBox[0])):
      ...
      for adjBox in grid.getAdjacentBoxes(curBox[0],lenX,lenY,lenZ): 
        ...
        figureOutEdge(curBox, curDist, oldEncode, currentBoxes, \
                      history, adjBox, 1)
      #also do side boxes
      ...
      for adjBox in grid.getCornerAdjacentBoxes(curBox[0],lenX,lenY,lenZ):
        ...
        figureOutEdge(curBox, curDist, oldEncode, currentBoxes, \
                      history, adjBox, 3.**.5)
      if extraEdges.has_key(curBox[0]):
        for adjBox,adjDist,adjGridDist in extraEdges[curBox[0]]: #tuple unpack
  ...
  # propagated to it
  pointTravelDist = grid.assignPointsValues(pointList, \
                           gridD, gridSize, mins, maxs, allPoints)
  if volumePoints:
    volumePointDepths = grid.assignPointsValues(volumePoints, \
                           gridD, gridSize, mins, maxs)
  if volumePoints:
