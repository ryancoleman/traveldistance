...
def fillInHolesAndGrow(
    triPointsOrig, pointTriListOrig, pointXyzOrig, normXyzOrig, numHandles,
    outFileName, debugOut, meshData, centerName):
  ...
  for countHandleRemoved in range(numHandles):
    ...
    while not regLoopOkay:
      ...
      if len(smallestList) > 0:
        ...
        while not regLoopOkay and len(smallestList) > smallestIndex:
          ...
          regLoopOkay = __checkRegularLoop(
              regLoops, regPts, smallest, trisRemoved, newTris)
          ...
          if regLoopOkay:  # do more checks
            ...
            ringNeighbors, ringToDisc, ringToExclude, ringToCenterName, \
                possHoleStarts = meshData.findOnePlug(
                    countHandleRemoved, regPts[smallest], debugOut,
                    outFileName, centerName)
            ...
            for ringCount, ring in enumerate(ringToDisc.keys()):
              ...
              for potentialMaxima in maximaDisc:
                if meshData.nodeClass[potentialMaxima] == 0:
                  #print "rejecting based on plug outside convex hull",
                  #print countHandleRemoved
                  regLoopOkay = False
        ...
        if regLoopOkay:  # passed all tests, so we are keeping this reg loop
          ...
          #possibleHoleStartsAll.extend(possHoleStarts)
      ...
      if altOffset > 30:  # hacky kludge to get results
        ...
        break
    #found one to change...
    #add it to list to be returned in ascending order of size
    #print regKinds[smallest]
    for index in xrange(len(returnLoops)+1):
      if index == len(returnLoops):
        returnLoops.append(regLoops[smallest])
        returnPoints.append(regPts[smallest])
        #stops anyway after this iteration
    ...
    #so we detect here and remove them with the loop to maintain topological
    # correctness
    trisToRemove = regLoops[smallest][:]  # copy
    loopsToRemove = [regPts[smallest][0][:], regPts[smallest][1][:]]  # copyboth
    #print len(triPoints), len(pointTriList), len(trisRemoved), len(newTris)
    __removeLoop(
        loopsToRemove, trisToRemove, pointTriList, triPoints, trisRemoved,
        newTris, debugOut, outFileName, pointXyz, countHandleRemoved)
    #print len(triPoints), len(pointTriList), len(trisRemoved), len(newTris)
  ...
  for ringCount, ring in enumerate(ringToDiscAll.keys()):
    biggestMaxima = meshData.growOneMaxima(
        ringToMaximaAll, ring, ringToPotentialMaximaAll[ring], centerName,
        ringToExcludeAll, pointNeighbors, outsidePoints, ringNeighborsAll,
        ringToDiscAll)
    if biggestMaxima:
  ...
  return loopTrisSave, loopPtsSave, returnLoops, returnPoints, pointNeighbors, \
      pointNeighborsOut, outsidePoints, outsidePointsOut, possHoleStarts

def __makeOffsetOrder(length):
  originalList = [xVal for xVal in xrange(length)]
  orderList, counter = [], 0
...
def __getListRegularLoop(
    triPoints, pointTriList, pointXyz, altOffset,
  ...
  if debugOut:
    tstdebug.debugTriangleList(
        loops, triPoints, pointXyz, outFileName +
        ".orig.loops." + str(countHandleRemoved) + ".py")
  #print len(loops), (numHandles - countHandleRemoved) * 2
  #no longer necessary, checks implemented
  loopPoints, loopsNew, regLoops, regPts, regKinds = regularizeLoops(
      triPoints, pointTriList, pointXyz, normXyz, loops)
  if debugOut:
    tstdebug.debugTriangleList(
        loopsNew, triPoints, pointXyz, outFileName +
        ".loops." + str(countHandleRemoved) + ".py")
    tstdebug.debugTriangleList(
        regLoops, triPoints, pointXyz,
        outFileName + ".regular.loops." + str(countHandleRemoved) + ".py")
  ...
  smallest = []
  #now a list of possibilities sorted in ascending order of length
...
def __checkRegularLoop(regLoops, regPts, oneToCheck, trisRemoved, newTris):
  ...
  if regLoopOkay:  # no point in checking if already bad
    loopsToCheck = [regPts[oneToCheck][0][:], regPts[oneToCheck][1][:]]
    #copy both
...
def __removeLoop(
    loopsToRemove, trisToRemove, pointTriList, triPoints,
  ...
  if loopsToRemove:
        ...
        for thisPt in checkLoop:
          for triToCheck in pointTriList[thisPt-1][2:]:
            ...
            if lastPt in points and secondLastPt in points:
              ...
              checkLoop.remove(lastPt)
              #basically adding tri to loop and removing one point
    ...
    if debugOut:
      tstdebug.debugTriangleList(
          [list(trisToRemove)], triPoints, pointXyz,
          outFileName + ".removeTris." + str(countHandleRemoved) + ".py")
    ...
    for loopIndex, vertexLoop in enumerate(loopsToRemove):
      trisToAdd = __trianglinizeLoopNoDupes(
          vertexLoop, triPoints, pointTriList)
      if debugOut:
        tstdebug.debugTrianglesNotOrig(
            trisToAdd, pointXyz,  outFileName + ".addTris." +
            str(countHandleRemoved) + "." + str(loopIndex) + ".py",
            ptList=list(vertexLoop))
...
def fillInHoles(
    triPointsOri, pointTriListOri, pointXyzOri, normXyzOri,
  ...
  for countHandleRemoved in range(numHandles):
    ...
    while not regLoopOkay:
      ...
      regLoops, regPts, smallestList, regLoopOkay, loopTris, loopPts = \
          __getListRegularLoop(
              triPoints, pointTriList, pointXyz, altOffset, debugOut,
              countHandleRemoved, outFileName, normXyz, trisRemoved, newTris)
      if 0 == countHandleRemoved:
        loopTrisSave = loopTris
        loopPtsSave = loopPts
      altOffset += 1
    ...
    #sometimes a triangle is made of 3 points adjacent in the loop... this is
    #a big problem later (makes edges with more than 2 triangles)
    #so we detect here and remove them with the loop to maintain
    #topological correctness
    trisToRemove = regLoops[smallest][:]  # copy
    loopsToRemove = [regPts[smallest][0][:], regPts[smallest][1][:]]
    #print len(triPoints), len(pointTriList), len(trisRemoved), len(newTris)
    __removeLoop(
        loopsToRemove, trisToRemove, pointTriList, triPoints, trisRemoved,
        newTris, debugOut, outFileName, pointXyz, countHandleRemoved)
  ...
  while not everythingOkay:
    ...
    for tri in possibleTrisToAdd:
      for edge in makeAllEdges(tri):
        otherTris = findAllTrisAcrossEdge(
            edge, allTris, pointTriList, triPoints)
        if len(otherTris) > 1:
          #2 or greater is bad, means more than 2 tris per edge
          everythingOkay = False
...
def fillInHolesTst(tstd, numHandles, outFileName, debugOut):
  ...
  return fillInHoles(
      triPoints, pointTriList, pointXyz, normXyz, numHandles, outFileName,
      debugOut)

def expandDisc(
    triPoints, pointTriList, pointXyz, alternateStartOffset=0,
  ...
  first = len(triPoints) - alternateStartOffset
  #start at end, avoids new added tris better
  while triPoints[first-1] == [False]:
    ...
    if first <= 0:
      ...
      sys.exit(1)
  #print first,
  discTris, availableTris = set([first]), priodict.priorityDictionary()
  availTrisToEdge = {}
  ...
  if verboseDebug:
    ...
    tstdebug.debugTriangles(
        discTris, triPoints, pointXyz,
        strNum + "." + uniqueStr+verboseDebugFileName,
        ptList=clockwiseBoundaryPoints, triColor=triColor)
  ...
  while len(availableTris) > 0:
    #print len(clockwiseBoundaryPoints), len(discTris), len(unseenTris),
    #print len(availableTris)  # debug
    ...
    if not safeToAdd:  # still possibly okay, as long as exactly one edge is in
      ...
      if 1 == numberEdgesInBoundary:  # exactly 1
        safeToAdd = True
    #now it is either safe or not,
    # proceed with updates and prepare for next iteration
    if not safeToAdd:
      #really we don't know if these are okay unless we try to add all
      ...
      for otherEdge in otherEdges:
        unmarkTri = findTriAcrossEdge(
            otherEdge, stripTris, pointTriList, triPoints)
        if -1 == unmarkTri:
          unmarkTri = findTriAcrossEdge(
              otherEdge, unseenTris, pointTriList, triPoints)
        if -1 == unmarkTri:
          unmarkTri = findTriAcrossEdge(
              otherEdge, availableTris.keys(), pointTriList, triPoints)
        if unmarkTri > -1:  # else it is in the disc already
          combDistance = lengthAcrossTris(
              unmarkTri, nextPossibleTri, otherEdge, triPoints, pointXyz)
          combDistance += curDistance
          if unmarkTri not in availableTris or \
              combDistance < availableTris[unmarkTri]:
            availableTris[unmarkTri] = combDistance  # only go on here 3 times
      ...
      if 0 == numberEdgesInBoundary:  # insertion case
        ...
        try:
          ...
          tstdebug.debugTriangleList(
              [list(availableTris.keys())], triPoints, pointXyz,
              "debug.availableTris.py")
          tstdebug.debugTriangleList(
              [list(stripTris)], triPoints, pointXyz,
              "debug.stripTris.py")
          tstdebug.debugTriangleList(
              [list(discTris)], triPoints, pointXyz, "debug.discTris.py",
              ptListList=[clockwiseBoundaryPoints])
          tstdebug.debugTriangleList(
              [[nextPossibleTri]], triPoints, pointXyz, "debug.nextTri.py")
          sys.exit(1)
        ...
        tryDelete1 = (deletion - 1 + len(clockwiseBoundaryPoints)) % \
            len(clockwiseBoundaryPoints)
        tryDelete1Value = clockwiseBoundaryPoints[tryDelete1]
        tryDelete2 = (deletion + 1 + len(clockwiseBoundaryPoints)) % \
            len(clockwiseBoundaryPoints)
...
def expandLoop(
    loop, boundaries, triPoints, pointTriList, pointXyz,
    verboseDebug=False, verboseDebugFileName="temp.py"):
  '''takes a loop, expands both sides, when they touch, return the tracebacks'''
  loopTris, availableTris = set(loop), \
      [priodict.priorityDictionary(), priodict.priorityDictionary()]
  availableTriEdges = [{}, {}]  # the edges crossed to get to tris
  ...
  if verboseDebug:
    ...
    fractionSeen = float(numberTri)/float(len(triPoints))
    triColor = (1-fractionSeen, fractionSeen, 0.)
    tstdebug.debugTrianglesPtList(
        loopTris, triPoints, pointXyz,
        strNum+"."+uniqueStr+verboseDebugFileName,
        ptListList=clockwiseBoundaryPointList, triColor=triColor)
  for loopTri in loop:
    ...
    for current in clockwiseBoundaryPts:
      ...
      if -1 != adjTri:
        ...
        if adjTri in availableTris[other]:  # already have crossover
          #print "crossover1"
          return __crossoverReportLoop(
              adjTri, prevTri, tracebacks, loop, triPoints, pointTriList,
              pointXyz)
        combinedLength = lengthAcrossTris(
            prevTri, adjTri, edge, triPoints, pointXyz)
        if adjTri not in availableTris[side] or \
          ...
          availableTris[side][adjTri] = combinedLength
          #would add current if not 0
          availableTriEdges[side][adjTri] = edge
          #otherwise something faster already seen
  side = 0
  #side is set to which side is being processed,
  # other is set to the other obviously
  stripTris = set()  # the ones that can't be added yet or at all
  #all these need to be set before entering loop
  #side, stripTris, unseentris, clockwiseBoundaryPointList, loopTris,
  # availableTris
  #stopping criteria is that we've tried to add all possible tris on border
  while len(availableTris[0]) > 0 or len(availableTris[1]) > 0:
    ...
    #print len(availableTris[0]), len(availableTris[1]), len(unseenTris),
    #print smallest
    ...
    if otherPoint in clockwiseBoundaryPointList[other]:
      ...
      for otherEdge in otherEdges:
        if __checkBoundaryEdge(clockwiseBoundaryPointList[other], otherEdge):
          otherTri = findTriAcrossEdge(
              otherEdge, loopTris, pointTriList, triPoints)
          countOtherEdges += 1
      #print countOtherEdges
      if countOtherEdges > 0:
        prevTri = findTriAcrossEdge(nextEdge, loopTris, pointTriList, triPoints)
        #print "crossover2", countOtherEdges, prevTri, nextPossibleTri,
        #print otherTri
        #tstdebug.debugTriangleList([tracebacks.keys()], triPoints, pointXyz,
        #    "temp.built.up.to.this.py")
        return __crossoverReportLoop(
            nextPossibleTri, otherTri, tracebacks, loop, triPoints,
            pointTriList, pointXyz)
    try:
    ...
    if -1 == found:  # other point not in boundary, safe to add, easy case
      ...
      numberEdgesInBoundary = 0
    if not safeToAdd:
      #still possibly okay, as long as exactly one edge is in
      numberEdgesInBoundary = 0
      for otherEdge in otherEdges:
        ...
        if unmarkTri > -1:
          ...
          if unmarkTri not in availableTris[side] or \
            ...
            if unmarkTri in availableTris[side]:
              #print unmarkTri in availableTris[side],
              #print oldMetric + combinedLength
              del availableTris[side][unmarkTri]
            availableTris[side][unmarkTri] = oldMetric + combinedLength
            #can only go on here 3 times
            otherEdgeTemp = otherEdge[:]
            otherEdgeTemp.reverse()
            availableTriEdges[side][unmarkTri] = otherEdgeTemp
            #otherwise better way already seen
          if unmarkTri in availableTris[other]:
            #print "crossover3"
            return __crossoverReportLoop(
                unmarkTri, nextPossibleTri, tracebacks, loop, triPoints,
                pointTriList, pointXyz)
          ...
          if unmarkTri in stripTris:
            ...
            tstdebug.debugTriangleList(
                [list(availableTris[side].keys())], triPoints, pointXyz,
                "debug.availableTris.py")
            tstdebug.debugTriangleList(
                [list(stripTris)], triPoints, pointXyz, "debug.stripTris.py")
            tstdebug.debugTriangleList(
                [[unmarkTri]], triPoints, pointXyz, "debug.unmarkTri.py")
            sys.exit(1)
      #also update boundary points, 2 cases
      if 0 == numberEdgesInBoundary:  # insertion case
        #add the other point in between the two
        ...
        try:
          ...
          sys.exit(1)
        #delete the one before or after the otherpoint that is in the tri
        tryDelete1 = (deletion - 1 + len(clockwiseBoundaryPointList[side])) % \
            len(clockwiseBoundaryPointList[side])
        tryDelete1Value = clockwiseBoundaryPointList[side][tryDelete1]
        tryDelete2 = (deletion + 1 + len(clockwiseBoundaryPointList[side])) % \
            len(clockwiseBoundaryPointList[side])
...
def getLoops(
    triPoints, pointTriList, pointXyz, alternateStartOffset=0):
  #print "getLoops start", len(triPoints), len(pointTriList),
  #print alternateStartOffset
  stripTris, boundaryPts, discTris = expandDisc(
      triPoints, pointTriList, pointXyz,  alternateStartOffset)
  #print len(stripTris), len(boundaryPts), len(discTris),
  seenTris = set()  # stores the ones we've seen already
  curTri = stripTris.pop()
  seenTris.add(curTri)
  tree, loopConnections = makeTreeBranch(
      curTri, seenTris, stripTris, triPoints, pointTriList)
...
def getLoopsAndVertices(triPoints, pointTriList, pointXyz):
  ...
  vertexLoops = []
  #tstdebug.debugTriangleList(
  #    loops, triPoints, pointXyz, "temp.original.loops.py")
  for index, loop in enumerate(loops):
    #tstdebug.debugTriangleList(
    #    [loop], triPoints, pointXyz, "temp.original."+str(index)+".loop.py")
...
def regularizeLoops(triPoints, pointTriList, pointXyz, normXyz, loops):
  ...
  for index, loop in enumerate(loops):
    boundaries = getVertexBoundary(
        loop, pointTriList, triPoints, pointXyz, returnBoth=True)
    if not boundaries:  # special failure case
      pass
    else:
  ...
  for index in range(len(loopsSave)):
    regularLoop = expandLoop(
        loopsSave[index], vertexLoopsSave[index], triPoints, pointTriList,
        pointXyz)
    if regularLoop:  # more error checking involved here
      loopBoundaryPts = getVertexBoundary(
          regularLoop, pointTriList, triPoints, pointXyz, returnBoth=True)
      if not loopBoundaryPts:  # special failure case
        pass  # used to report, no longer a big deal
      else:
  ...
  for index in range(len(loopsSave2)):
    kind = classifyRegularLoop(
        regularLoops[index], regularLoopPts[index], triPoints, pointXyz)
    kinds.append(kind)
...
def classifyRegularLoop(loopTris, loopPts, triPoints, pointXyz):
  ...
  for loopTri in loopTris:
    ...
    normal = geometry.getTriNormal(
        triPointXYZ[0], triPointXYZ[1], triPointXYZ[2])
...
def getVertexBoundary(
    loop, pointTriList, triPoints, pointXyz=False, returnBoth=False):
  loopSet = set(loop)
  boundaryLists = [[], []]
  for triangle in loop:
    ...
    for edge in edges:
      adjTri = findTriAcrossEdge(
          edge, loopSet, pointTriList, triPoints, notThisTris=set([triangle]))
      if -1 == adjTri:
        if nonAdjacentEdge:
          print "serious error in loop structure 1"
          print loop, triangle
          tstdebug.debugTriangleList(
              [loop], triPoints, pointXyz, "crash.loop.py")
          tstdebug.debugTriangleList(
              [[triangle]], triPoints, pointXyz, "crash.tri.py")
          sys.exit(1)
        nonAdjacentEdge = edge
    if not nonAdjacentEdge:
      #if pointXyz != False:
        #print triangle
        #tstdebug.debugTriangleList(
        #    [[triangle]], triPoints, pointXyz, "crash.tri.py")
    ...
    for boundaryList in boundaryLists:
      for point in nonAdjacentEdge:
        if len(boundaryList) > 0 and boundaryList[-1] == point:
  ...
  if 0 == len(boundaryLists[0]) or 0 == len(boundaryLists[1]):
    return False
  #otherwise proceed...
  ...
  while len(stackTreesTraversed) > 0:
    ...
    if 1 == len(curTree):  # leaf
      if curTree[0] in connectionTargets:
        ...
        #dictionary keyed on connectionTarget
...
def makeTreeBranch(
    firstTri, seenTris, unseenTris, triPoints, pointTriList, lastTri=False):
  '''
  tree making function  ---changed from recursion to iteration
  '''
  loopConnections = {}
  #print "mtb", firstTri, len(seenTris), len(unseenTris), len(triPoints),
  #print len(pointTriList), len(loopConnections), lastTri,
  tree = ['replace']
  stackToProcess = [[firstTri, lastTri, seenTris, unseenTris, tree, 0]]
  #this starts the stack
  while len(stackToProcess) > 0:
    firstTri, lastTri, seenTris, unseenTris, treeInsert, point = \
        stackToProcess.pop()
    if 0 == len(unseenTris):
    ...
    for edge in edges:
      ...
      if -1 != adjTri:
        ...
        connectionTri = findTriAcrossEdge(
            edge, seenTris, pointTriList, triPoints,
            notThisTris=set([lastTri, firstTri]))
    ...
    if 0 == len(adjacentTris):
      ...
      stackToProcess.insert(
          0, [nextTri, firstTri, seenTris, unseenTris, treeInsert[point], 1])
      #the 'replace'
    elif 2 == len(adjacentTris):
      #splitting, either at a 3-way or just starting
      for nextTri in adjacentTris:
        unseenTris.remove(nextTri)
        seenTris.add(nextTri)
        #both these must be done before recursive calls
      treeInsert[point] = [firstTri, 'replace1', 'replace2']
      for index, nextTri in enumerate(adjacentTris):
        stackToProcess.insert(
            0, [
                nextTri, firstTri, seenTris, unseenTris, treeInsert[point],
                index+1])  # the 'replace'
    elif 3 == len(adjacentTris):
      # very bad... messy for data structure , restart
      newStartTri = unseenTris.pop()
      unseenTris.extend(seenTris)
      seenTris.clear()
      return makeTreeBranch(
          newStartTri, seenTris, unseenTris, triPoints, pointTriList)
      #just give up and start over... don't want degree 3 vertices
  return tree[0], loopConnections
  #dumb but necessary to return tree[0] not tree
...
def __checkBoundaryCounterEdge(clockwiseBoundaryPoints, counterEdge):
  ...
  check = (found - 1 + len(clockwiseBoundaryPoints)) % \
      len(clockwiseBoundaryPoints)
...
def makeOtherEdges(possPoints, nextEdge):
  ...
  return [
      [possPoints[finalIndex], possPoints[nextIndex]],
...
def makeAllEdges(possPoints):
  ...
  '''
  return [
      [possPoints[0], possPoints[1]], [possPoints[1], possPoints[2]],
      [possPoints[2], possPoints[0]]]

def findTriAcrossEdge(
    edge, validTris, pointTriList, triPoints, notThisTris=set()):
  '''
  validTris is a set
  '''
  nextPossibleTri = findAllTrisAcrossEdge(
      edge, validTris, pointTriList, triPoints, notThisTris)
  if [] == nextPossibleTri:
    return -1
  else:
...
def findAllTrisAcrossEdge(
    edge, validTris, pointTriList, triPoints, notThisTris=None):
  if notThisTri is None:
    notThisTri = set()
...
def __crossoverReportLoop(
    crossoverTri, otherTri, tracebacks, oldLoop, triPoints, pointTriList,
    pointXyz):
  ...
  if loop[0] == loop[-1]:
    ...
    if abs(start-end) < abs(len(oldLoop)-(max(start, end))) + \
        abs(min(start, end)):
    ...
    for edge in edges:
      ...
      if adjTri != loop[next] and adjTri != loop[prev]:
        if adjTri in oldLoopSet:
          ...
          if len(returnLoop) > 1:
            #returnloop should be okay...
            # but possible we have 2 places wrong.. so
            #call this loop again...
            #print "problem found and fixed once"
            #print returnLoop
            return __possiblyTrimLoop(
                returnLoop, oldLoop, triPoints, pointTriList, pointXyz)
...
def lengthAcrossTris(triOne, triTwo, adjEdge, triPoints, pointXyz):
  ...
  edges = [
      makeAllEdges(triPoints[triOne-1][1:]),
      makeAllEdges(triPoints[triTwo-1][1:])]
  farPoints = []
  for tris in edges:
    pts = [
        pointXyz[tris[0][0]-1][1:], pointXyz[tris[1][0]-1][1:],
