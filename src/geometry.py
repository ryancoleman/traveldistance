...
useNumpy = False
try: #to use numeric
  import Numeric, Matrix, LinearAlgebra
  ...
  try:
    ...
    import pMatrix #http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/189971
...
#math from Eric W. Weisstein. "Line-Plane Intersection." From MathWorld--A Wolfram Web Resource. http://mathworld.wolfram.com/Line-PlaneIntersection.html
# t = - |1  1  1  1 |
#       |x1 x2 x3 x4|
#       |y1 y2 y3 y4|
#       |z1 z2 z3 z4|
...
def linePlaneIntersection(p1,p2,p3,p4,p5):
  top = pMatrix.pMatrix([[1.,1.,1.,1.],[p1[0],p2[0],p3[0],p4[0]],[p1[1],p2[1],p3[1],p4[1]],[p1[2],p2[2],p3[2],p4[2]]])
  topDet = top.determinant()
  bottom = pMatrix.pMatrix([[1.,1.,1.,0.],[p1[0],p2[0],p3[0],p5[0]-p4[0]],[p1[1],p2[1],p3[1],p5[1]-p4[1]],[p1[2],p2[2],p3[2],p5[2]-p4[2]]])
...
def linePlaneIntersectionNumeric(p1,p2,p3,p4,p5):
  ...
  if useNumpy:
    top = [[1.,1.,1.,1.],[p1[0],p2[0],p3[0],p4[0]],[p1[1],p2[1],p3[1],p4[1]],[p1[2],p2[2],p3[2],p4[2]]]
    topDet = numpy.linalg.det(top)
    bottom = [[1.,1.,1.,0.],[p1[0],p2[0],p3[0],p5[0]-p4[0]],[p1[1],p2[1],p3[1],p5[1]-p4[1]],[p1[2],p2[2],p3[2],p5[2]-p4[2]]]
    botDet = numpy.linalg.det(bottom)
  else: #actually use numeric
    top = Matrix.Matrix([[1.,1.,1.,1.],[p1[0],p2[0],p3[0],p4[0]],[p1[1],p2[1],p3[1],p4[1]],[p1[2],p2[2],p3[2],p4[2]]])
    topDet = LinearAlgebra.determinant(top)
    bottom = Matrix.Matrix([[1.,1.,1.,0.],[p1[0],p2[0],p3[0],p5[0]-p4[0]],[p1[1],p2[1],p3[1],p5[1]-p4[1]],[p1[2],p2[2],p3[2],p5[2]-p4[2]]])
...
#helper function that checks to see if the intPt is inside the triangle p1,p2,p3
  ...
  try:
    ...
    for coord in range(3):
      for which in range(2):
        outputPoints[which].append(minLine[coord] + \
                                   dirLine[coord]*intersections[which])
...
def countPathTriIntersections(pathPoints, triangle):
  ...
  for nextPathPt in pathPoints[1:]:
    ...
    while False == posPt:
      posPt = linePlaneIntersectionNumeric(triPts0,triPts1,triPts2,lastPathPt,nextPathPt)
      if False == posPt:
        triPts0,triPts1,triPts2 = perturbTriangle(triPts0,triPts1,triPts2)
        maxIt -= 1
        if maxIt < 0:
          print "had to perturb points 5000 times",triPts0,triPts1,triPts2,lastPathPt,nextPathPt,"giving up"
          sys.exit(1)
    if posPt is not False:
      if distL2(lastPathPt, nextPathPt) >= distL2(lastPathPt, posPt) and \
        ...
        if intPointInsideTri(triPts0,triPts1,triPts2,posPt): #broken  when using large tri?
          intersectionCount += 1
...
def perturbLine(longAxis, shortAxis1, shortAxis2, startPt, endPt, itersLeft):
    ...
    if itersLeft % 4 == 3: #alternate back and forth around line
      newStartPt[shortAxis1] = startPt[shortAxis1]+float(0.0000000001*(5001.-itersLeft))
      newStartPt[shortAxis2] = startPt[shortAxis2]-float(0.000000001*(5001.-itersLeft))
      newEndPt[shortAxis1] = endPt[shortAxis1]+float(0.00000001*(5001.-itersLeft))
      newEndPt[shortAxis2] = endPt[shortAxis2]-float(0.000000001*(5001.-itersLeft))
    elif itersLeft % 4 == 2: #alternate back and forth around line
      newStartPt[shortAxis1] = startPt[shortAxis1]-float(0.0000001*(5001.-itersLeft))
      newStartPt[shortAxis2] = startPt[shortAxis2]+float(0.000000001*(5001.-itersLeft))
      newEndPt[shortAxis1] = endPt[shortAxis1]+float(0.00000001*(5001.-itersLeft))
      newEndPt[shortAxis2] = endPt[shortAxis2]-float(0.000000001*(5001.-itersLeft))
    elif itersLeft % 4 == 1: #alternate back and forth around line
      newStartPt[shortAxis1] = startPt[shortAxis1]+float(0.0000000001*(5001.-itersLeft))
      newStartPt[shortAxis2] = startPt[shortAxis2]-float(0.000001*(5001.-itersLeft))
      newEndPt[shortAxis1] = endPt[shortAxis1]-float(0.0000001*(5001.-itersLeft))
      newEndPt[shortAxis2] = endPt[shortAxis2]+float(0.0000000001*(5001.-itersLeft))
    else:
      newStartPt[shortAxis1] = startPt[shortAxis1]-float(0.0000001*(5001.-itersLeft))
      newStartPt[shortAxis2] = startPt[shortAxis2]+float(0.0000001*(5001.-itersLeft))
      newEndPt[shortAxis1] = endPt[shortAxis1]-float(0.000000001*(5001.-itersLeft))
      newEndPt[shortAxis2] = endPt[shortAxis2]+float(0.00000001*(5001.-itersLeft))
...
def getLongestEdge(triList, pointList,direction=-1):
  ...
  if -1 == direction:
    for triangle in triList:
      distAB = distL2(pointList[triangle[1]-1][1:], \
                             pointList[triangle[2]-1][1:])
      distBC = distL2(pointList[triangle[2]-1][1:], \
                             pointList[triangle[3]-1][1:])
      distCA = distL2(pointList[triangle[3]-1][1:], \
                             pointList[triangle[1]-1][1:])
...
def cacheTriangle(triList, pointList, allowedTris=[-1]):
  ...
  # the tuple format is ((x),(y),(z),(x-y),(y-x),(y-z),(z-y),(x-z),(z-x),(tri#))
