...
borderStyleDict = {
    0: 'solid', 1: 'dashed', 2: 'dotted', 3: 'double', 4: 'triple'}
...
def outputDrawStr(treeName, node):
  ...
  return "tstOpenDraw " + makeTstStr(treeName) + ",group=" + node.getId()
...
class dot(object):
  ...
  class subgraph(object):
    ...
    def gdlStr(
        self, colornumber=0, edges=True, edgeColor="lightgrey",
      ...
      for parent, children in self.tree.iteritems():
        ...
        for child in children:
          ...
          if edges and (justNodes is None):
            retStr += " edge: { source: \"" + parent + "\" target: \"" + \
                child + "\" color: " + edgeColor + " thickness: 1 }\n"
      retStr += " }\n"
      return retStr
  ...
  def __init__(self, tmDataList):
    ...
    for tmDataCount, tmData in enumerate(tmDataList):
      ...
      newSubgraph = self.subgraph(
          tmData.inputFileName, tmData.tree,
  ...
  def addConnections(
      self, columnListNames, threshold, remove=True, resIds=False):
    ...
    for tmDataCount, tmData in enumerate(self.tmDataList[1:]):
      ...
      for connection in tempConns:
        if connection[2] <= self.threshold:
          newConn = [
              subGraph1.nodeToName[connection[0]],
  ...
  def computeSearchConnections(
      self, threshold, columnList, columnsToMean, columnsToStddev,
          ...
          for tmNodeCount, node1 in enumerate(tmData.tree.keys()):
            if justNodes is None or node1 in justNodes:
              for tmNode2Count, node2 in enumerate(tmData2.tree.keys()):
                if (
                    (
                        tmDataCount < tmDataCount2 or
                        tmNodeCount < tmNode2Count) and
                  ...
                  if score < threshold:
                    if tmDataCount == tmDataCount2:  # same protein struct/tree
                      ...
                      if score2 < 100.:  # if there is overlap within same prot
                        okayWithin = False
                    if okayWithin:  # no res overlap
                      ...
                      if doSelfScore:  # do this instead
                        totalScore = -1./score
                        totalScore += alpha * (
                            selfScores[node1] + selfScores[node2])
                      matchList.append(
                          (tmData, tmData2, node1, node2, totalScore))
                      if returnMatrix:
  ...
  def tempRemoveKeepersWriteGdl(
      self, outputFile, force=True, edges=True, justNodes=None):
    ...
    for subGraph in self.tmToSubgraph.values():
      ...
      subGraph.removeNonKeepers()
    self.writeGdl(
        outputFile, force, edges=edges, justNodes=justNodes,
  ...
  def writeGdl(
    ...
    if colorByResidueTanimoto:
      ...
      nodeToMeanTanimoto = collections.defaultdict(float)  # defaults  0 overlap
    ...
    for sgnum, tmTree in enumerate(self.tmDataList):
      ...
      if colorByResidueTanimoto:
        outFile.write(
            subgraph.gdlStr(
                sgnum, edges=edges, justNodes=justNodes,
    ...
    for connection in self.connections:
      ...
      #              label + "\" penwidth=" + str(width) + "];\n")
      outFile.write(
          "edge: { source: \"" + connection[0] + "\" target: \"" +
  ...
  def findChains(self):
    ...
    connDict = {}
    for connection in self.connections:  # rely on 0->1 ordering / consistent
    ...
    printCols = self.tmToSubgraph.keys()[0].titlesToColumns(
        ["Volume", "meanTD"])
...
class dotResidues(dot):
  ...
  def __init__(
      self, tmDataList, residueList, threshold=50,
  ...
  def writeAnimation(
      self, outFileName="temp.animation.py", width=3, bb=False, maxTD=True):
    ...
    for index, tm3tree in enumerate(self.tmDataList):
      ...
      if not bb:
        outFile.write(
            "png temp." + str(index).zfill(width) + "." + tstName + ".png")
      else:
        outFile.write(
            "png temp.bb." + str(index).zfill(width) + "." + tstName + ".png")
      outFile.write("\")\n")
    outFile.close()
...
def compareColumnsResidues(tmNode1, tmNode2, columnList):
  ...
  if union > 0.:  # otherwise automatically no match
    ...
    return 100 - (float(len(resIds1.intersection(resIds2)))/union) * 100.
    #want complete match to be equal to 0
  else:
