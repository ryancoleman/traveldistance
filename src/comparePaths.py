...
def readCGOPathRadius(filename):
  ...
  for index in sphereIndices:
    result.append((float(data[index+1]),float(data[index+2]),float(data[index+3])))
    try:
...
def comparePaths(source=False, target=False, sourceDataManual=False, \
                 core=100., sourceRadiiManual=False):
  result = comparePathsCoverage(source,target,sourceDataManual,core,\
                                sourceRadiiManual)
  return result[0] #just rmsd

def comparePathsCoverage(source=False, target=False, \
                    sourceDataManual=False, core=100., sourceRadiiManual=False):
  '''get both the rmsd and how much of the target was seen'''
  result = comparePathsManyMetrics(source,target,sourceDataManual, \
                                   sourceRadiiManual, core)
  return result[:2] #just first two

def comparePathsManyMetrics(source=False, target=False, sourceDataManual=False,\
  ...
  for index,sourceDatum in enumerate(sourceData):
    ...
    if float(index)/float(len(sourceData)) >= outside and \
      ...
      dataCounted += 1.
      #find match
      closest,distance = targetData[0],distL2(targetData[0], sourceDatum)
      tarRad = 1.
      for tarIndex,targetDatum in enumerate(targetData):
        thisDist =  distL2(targetDatum, sourceDatum)
  ...
  if dataCounted > 0.:
    ...
    span = float(targetMappedToIndices[-1]-targetMappedToIndices[0]+1.)/float(len(targetData)) #percentage of length of target from first covered to last covered
