...
import sys, string, math #python
import tstdata, cavity, geometry #mine
...
def calculateArea(allTris, triPoints, pointXYZ):
  ...
  for tri in allTris:
    ...
    area += geometry.calcTriArea(pointXYZ[points[0]-1][1:], \
                       pointXYZ[points[1]-1][1:], pointXYZ[points[2]-1][1:])
  return area
...
def tstSurfVolAnalysis(tstFileName):
  tstD = tstdata.tstData(tstFileName, \
            necessaryKeys=tstdata.tstData.necessaryKeysForSV) #could be less
  ...
  allArea = calculateArea(surfTris, \
                       tstD.dict['TRIANGLE_POINT'], \
                       tstD.dict['POINT_XYZ'])
  allVol = calculateVolume(surfTris, \
                       tstD.dict['TRIANGLE_POINT'], \
                       tstD.dict['POINT_XYZ'])
  cavArea = calculateArea(cavTris, \
                       tstD.dict['TRIANGLE_POINT'], \
                       tstD.dict['POINT_XYZ'])
  cavVol = calculateVolume(cavTris, \
                       tstD.dict['TRIANGLE_POINT'], \
                       tstD.dict['POINT_XYZ'])
  chArea = False
  if chVol:
    chArea = calculateArea(chTris, tstD.dict['CONVEX_HULL_TRI_POINT_LIST'], \
                           tstD.dict['POINT_XYZ'])
    chVol = calculateVolume(chTris, tstD.dict['CONVEX_HULL_TRI_POINT_LIST'], \
                           tstD.dict['POINT_XYZ'])
